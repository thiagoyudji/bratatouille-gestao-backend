...
public class PartnersApiController implements PartnersApi {
    ...
    private final PartnersApiDelegate delegate;

    public PartnersApiController(@Autowired(required = false) PartnersApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new PartnersApiDelegate() {});
    }

    @Override
    public PartnersApiDelegate getDelegate() {
        return delegate;
    }
