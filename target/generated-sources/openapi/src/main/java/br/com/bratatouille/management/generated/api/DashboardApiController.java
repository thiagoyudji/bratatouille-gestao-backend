...
public class DashboardApiController implements DashboardApi {
    ...
    private final DashboardApiDelegate delegate;

    public DashboardApiController(@Autowired(required = false) DashboardApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new DashboardApiDelegate() {});
    }

    @Override
    public DashboardApiDelegate getDelegate() {
        return delegate;
    }
