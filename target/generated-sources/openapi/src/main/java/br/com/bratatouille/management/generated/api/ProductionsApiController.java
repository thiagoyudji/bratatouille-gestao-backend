...
public class ProductionsApiController implements ProductionsApi {

    private final ProductionsApiDelegate delegate;

    public ProductionsApiController(@Autowired(required = false) ProductionsApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new ProductionsApiDelegate() {});
    }

    @Override
    public ProductionsApiDelegate getDelegate() {
        return delegate;
    }
