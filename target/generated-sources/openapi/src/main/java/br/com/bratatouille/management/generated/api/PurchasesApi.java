/**
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech) (7.14.0).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
package br.com.bratatouille.management.generated.api;

import br.com.bratatouille.management.generated.model.PurchaseCreateRequest;
import br.com.bratatouille.management.generated.model.PurchaseResponse;
import io.swagger.v3.oas.annotations.ExternalDocumentation;
import io.swagger.v3.oas.annotations.Operation;
import io.swagger.v3.oas.annotations.Parameter;
import io.swagger.v3.oas.annotations.Parameters;
import io.swagger.v3.oas.annotations.media.ArraySchema;
import io.swagger.v3.oas.annotations.media.Content;
import io.swagger.v3.oas.annotations.media.Schema;
import io.swagger.v3.oas.annotations.responses.ApiResponse;
import io.swagger.v3.oas.annotations.security.SecurityRequirement;
import io.swagger.v3.oas.annotations.tags.Tag;
import io.swagger.v3.oas.annotations.enums.ParameterIn;
import io.swagger.v3.oas.annotations.media.ExampleObject;
import org.springframework.lang.Nullable;
import org.springframework.http.ResponseEntity;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.multipart.MultipartFile;

import jakarta.validation.Valid;
import jakarta.validation.constraints.*;
import java.util.List;
import java.util.Map;
import jakarta.annotation.Generated;

@Generated(value = "org.openapitools.codegen.languages.SpringCodegen", date = "2026-08-13T16:34:02.160723012-03:00[America/Sao_Paulo]", comments = "Generator version: 7.14.0")
@Validated
@Tag(name = "Purchases", description = "the Purchases API")
public interface PurchasesApi {

    default PurchasesApiDelegate getDelegate() {
        return new PurchasesApiDelegate() {};
    }

    /**
     * POST /api/purchases : Create purchase
     *
     * @param purchaseCreateRequest  (required)
     * @return Purchase created (status code 200)
     */
    @Operation(
        operationId = "createPurchase",
        summary = "Create purchase",
        tags = { "Purchases" },
        responses = {
            @ApiResponse(responseCode = "200", description = "Purchase created", content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = PurchaseResponse.class))
            })
        }
    )
    @RequestMapping(
        method = RequestMethod.POST,
        value = "/api/purchases",
        produces = { "application/json" },
        consumes = { "application/json" }
    )
    
    default ResponseEntity<PurchaseResponse> createPurchase(
        @Parameter(name = "PurchaseCreateRequest", description = "", required = true) @Valid @RequestBody PurchaseCreateRequest purchaseCreateRequest
    ) {
        return getDelegate().createPurchase(purchaseCreateRequest);
    }


    /**
     * GET /api/purchases : List all purchases
     *
     * @return Purchases found (status code 200)
     */
    @Operation(
        operationId = "findAllPurchases",
        summary = "List all purchases",
        tags = { "Purchases" },
        responses = {
            @ApiResponse(responseCode = "200", description = "Purchases found", content = {
                @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = PurchaseResponse.class)))
            })
        }
    )
    @RequestMapping(
        method = RequestMethod.GET,
        value = "/api/purchases",
        produces = { "application/json" }
    )
    
    default ResponseEntity<List<PurchaseResponse>> findAllPurchases(
        
    ) {
        return getDelegate().findAllPurchases();
    }


    /**
     * GET /api/purchases/{id} : Find purchase by id
     *
     * @param id  (required)
     * @return Purchase found (status code 200)
     */
    @Operation(
        operationId = "findPurchaseById",
        summary = "Find purchase by id",
        tags = { "Purchases" },
        responses = {
            @ApiResponse(responseCode = "200", description = "Purchase found", content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = PurchaseResponse.class))
            })
        }
    )
    @RequestMapping(
        method = RequestMethod.GET,
        value = "/api/purchases/{id}",
        produces = { "application/json" }
    )
    
    default ResponseEntity<PurchaseResponse> findPurchaseById(
        @Parameter(name = "id", description = "", required = true, in = ParameterIn.PATH) @PathVariable("id") Long id
    ) {
        return getDelegate().findPurchaseById(id);
    }

}
