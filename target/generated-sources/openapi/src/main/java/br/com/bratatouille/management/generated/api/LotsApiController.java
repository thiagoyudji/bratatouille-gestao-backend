...
public class LotsApiController implements LotsApi {

    private final LotsApiDelegate delegate;

    public LotsApiController(@Autowired(required = false) LotsApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new LotsApiDelegate() {});
    }

    @Override
    public LotsApiDelegate getDelegate() {
        return delegate;
    }
