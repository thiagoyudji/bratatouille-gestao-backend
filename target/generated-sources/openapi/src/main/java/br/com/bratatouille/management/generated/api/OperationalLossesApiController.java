...
public class OperationalLossesApiController implements OperationalLossesApi {

    private final OperationalLossesApiDelegate delegate;

    public OperationalLossesApiController(@Autowired(required = false) OperationalLossesApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new OperationalLossesApiDelegate() {});
    }

    @Override
    public OperationalLossesApiDelegate getDelegate() {
        return delegate;
    }
