...
public class FinancialApiController implements FinancialApi {

    private final FinancialApiDelegate delegate;

    public FinancialApiController(@Autowired(required = false) FinancialApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new FinancialApiDelegate() {});
    }

    @Override
    public FinancialApiDelegate getDelegate() {
        return delegate;
    }
